...
| **Backend API** | https://ambient-climate-backend-977755576323.europe-west6.run.app/api/v1/latest?device_id=m5stack-ana-home |
...
| `GET` | `/speech/meditation` | Returns a meditation session **generated by a gpt-4o-mini agent** (title, breath cadence, timed prompts). 5 themes via `?session=` (calm / sleep / focus / stress / energize); cached per theme, `?fresh=1` regenerates; falls back to the static catalog on failure |
...
- `meditation_agent` — **LLM-generated guided meditation.** Calls `gpt-4o-mini` (structured `json_schema` output, same pattern as `agent_service`) to write a varied script for one of 5 themes (calm / sleep / focus / stress / energize), then re-spaces the cue timings evenly across the session and caps the count for a meditative pace. Cached per theme (prompt ids are text-hash based so the device's TTS cache stays valid); any failure falls back to `meditation_service`
- `meditation_service` — static fallback catalog of guided meditation sessions (id, duration, breath cadence, timed text prompts). The device caches each prompt's TTS via `/speech/tts` on first run and plays them at scheduled times during the session
...
Guided session served by `/speech/meditation`. The script is **generated on the fly by a gpt-4o-mini meditation agent** (`meditation_agent.py`) rather than hand-written — each session carries a title, breath cadence, and a list of timed text prompts. On first run the device fetches each prompt as TTS audio via `/speech/tts?profile=m5stack` and caches them locally (`/sd/med_<id>.wav`); subsequent runs play offline.
...
**Five themes** are available — **Calm, Sleep, Focus, Stress Relief, and Energize** — each with its own duration and breath cadence. Tapping **Meditation** in the coach menu opens a picker (`MEDITATION_THEMES` in the firmware); the chosen theme is sent as `?session=<id>`. The agent writes a fresh script per theme, re-spaces the cues evenly across the session, and caches the result so prompt ids stay stable (the device's TTS cache keeps working). If generation fails for any reason, the backend falls back to the static `calm` script in `meditation_service.py`, so the device never stalls. Add a theme by extending `_THEMES` in `meditation_agent.py` and `MEDITATION_THEMES` in the firmware.
