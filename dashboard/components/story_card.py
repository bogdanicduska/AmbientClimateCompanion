...
def story_card(
    ...
    bullets_html = "".join(
        ...
        f'<span style="color:#778899;font-size:0.84rem;line-height:1.5;">{b}</span>'
        f'</div>'
        for b in bullets
    )

    headline_html = (
        f'<div style="font-size:0.92rem;color:#C8D8E8;font-weight:600;'
    ...
    st.markdown(
        ...
        f'<span style="font-size:0.68rem;color:#3A5A7A;letter-spacing:0.15em;'
        f'text-transform:uppercase;">Daily Room Story</span>'
        f'<span style="font-size:0.7rem;color:#253545;">{window_label}</span>'
        f'</div>'
        f'{headline_html}'
        f'{bullets_html}'
        f'</div>',
