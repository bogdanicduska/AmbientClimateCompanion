"""
Chart-building functions for the Room Rhythm dashboard.

Each function accepts a pandas DataFrame (already parsed, sorted by timestamp)
...
# ---------------------------------------------------------------------------
# Shared Plotly layout defaults — dark theme that matches the dashboard CSS
# ---------------------------------------------------------------------------
_PAPER_BG  = "#070C14"
_PLOT_BG   = "#0A1220"
_GRID      = "#0F1E2E"
...
def _threshold_line(fig, y: float, label: str, color: str = "#334455",
...
# ---------------------------------------------------------------------------
# 1. Room Performance Scores  (Readiness · Recovery · Air Strain)
# ---------------------------------------------------------------------------
def room_scores_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        x=t, y=df["room_readiness"],
        name="Room Readiness",
        mode="lines",
        line=dict(color="#3DFF8A", width=2),
        fill="tozeroy",
        fillcolor="rgba(61,255,138,0.04)",
        hovertemplate="%{y}<extra>Readiness</extra>",
    ))
    fig.add_trace(go.Scatter(
        x=t, y=df["recovery_score"],
        name="Recovery",
        mode="lines",
        line=dict(color="#66CCFF", width=2),
    ...
    fig.update_layout(
        **_base_layout(
            ...
            yaxis=dict(range=[0, 100], gridcolor=_GRID, showgrid=True,
                       tickcolor=_TICK, linecolor=_TICK, zeroline=False,
                       title="Score (0–100)", tickfont=dict(color=_FONT_CLR),
                       title_font=dict(color=_FONT_CLR)),
        )
    )
    return fig
...
def temperature_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_hrect(y0=18, y1=26, fillcolor="rgba(0,255,180,0.04)",
                  line_width=0, annotation_text="Comfort zone",
                  annotation_font=dict(color="#1A4A3A", size=10),
                  annotation_position="top right")
    ...
    fig.update_layout(
        **_base_layout(
            ...
            yaxis=dict(title="°C", gridcolor=_GRID, showgrid=True,
                       tickcolor=_TICK, linecolor=_TICK, zeroline=False,
...
def humidity_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color="#66CCFF", width=2),
        fill="tozeroy",
        fillcolor="rgba(102,204,255,0.04)",
...
def tvoc_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        x=df["timestamp"], y=df["air_quality"],
        name="TVOC",
        mode="lines",
    ...
    fig.update_layout(
        **_base_layout(
            ...
            yaxis=dict(title="ppb", gridcolor=_GRID, showgrid=True,
                       tickcolor=_TICK, linecolor=_TICK, zeroline=False,
                       tickfont=dict(color=_FONT_CLR), title_font=dict(color=_FONT_CLR)),
        )
    )
...
def eco2_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        x=df["timestamp"], y=df["indoor_eco2"],
        name="eCO₂",
        mode="lines",
    ...
    fig.update_layout(
        **_base_layout(
            ...
            yaxis=dict(title="ppm", gridcolor=_GRID, showgrid=True,
                       tickcolor=_TICK, linecolor=_TICK, zeroline=False,
                       tickfont=dict(color=_FONT_CLR), title_font=dict(color=_FONT_CLR)),
        )
    )
...
# ---------------------------------------------------------------------------
# 6. Motion / Occupancy activity
# ---------------------------------------------------------------------------
def motion_chart(df: pd.DataFrame) -> go.Figure:
    fig = go.Figure()

    # Step line so transitions are sharp (not interpolated)
    fig.add_trace(go.Scatter(
        x=df["timestamp"], y=df["motion"].astype(int),
        name="Motion",
        mode="lines",
        line=dict(color="#DDDD00", width=0, shape="hv"),
        fill="tozeroy",
        fillcolor="rgba(200,180,0,0.18)",
    ...
    fig.update_layout(
        **_base_layout(
            ...
            yaxis=dict(
                tickvals=[0, 1], ticktext=["None", "Detected"],
                range=[-0.1, 1.4],
                gridcolor=_GRID, showgrid=False,
                tickcolor=_TICK, linecolor=_TICK, zeroline=False,
                tickfont=dict(color=_FONT_CLR), title_font=dict(color=_FONT_CLR),
            ),
            legend=dict(
...
# ---------------------------------------------------------------------------
# Shared: parse and validate history DataFrame
# ---------------------------------------------------------------------------
