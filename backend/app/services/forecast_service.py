...
def _summarise_day(slots: List[Dict]) -> Dict[str, Any]:
    ...
    # Guard against missing 'weather' list in any slot
    storm = any(
        200 <= s["weather"][0]["id"] < 300
        for s in slots
        if s.get("weather")
    )
    first_weather = next((s["weather"][0] for s in slots if s.get("weather")), {})

    return {
        "date":             slots[0].get("dt_txt", "")[:10],
...
def fetch_forecast(config) -> Dict[str, Any]:
    ...
    for item in items:
        day_key = item.get("dt_txt", "")[:10]
        if day_key:
            days[day_key].append(item)

    summaries = [_summarise_day(days[k]) for k in sorted(days)[:3] if days[k]]
